...
<!-- Hide the code form counting the all task related information 2026-03-02 -->
...
<!-- akhila edited second-sreen-view start Inlude Footer here -->


		<div class="second-screen-view">
    ...
    <div class="form-group">
        ...
        <select class="form-control" id="project_manger" name="project_manger">
            ...
            $allowedManagers = [
                ...
                'siva krishna',
                'rahul kumar',
                'srinivas golanakonda',
                'syed afsar',
                'syed farhan',
                'pradip chauhan',
                'rajanikanth bhasuthkar',
                'nikhil bachawal'
            ];
...
<!-- akhila edited second-sreen-view ended Inlude Footer here -->
		...
		<div class="card-body">
            <div class="row">
                ...
                <div style="padding:8px; text-align:right;">
                    <a class="btn btn-success btn-flat" id="send_today_resource_email" data-toggle="tooltip" title="Send today's schedule"><i class="fa fa-paper-plane"></i> Sent</a> |
                    <a class="btn btn-primary btn-flat" id="downloadSearch_resouce_data" data-toggle="tooltip" title="Add"><i class="fa fa-lg fa-plus"></i> Download Today's Resource Schedule</a>
                    
                  <?php if(!empty($_REQUEST['showsearch']) !=1): ?>
                    
                    <!-- <a class="btn btn-primary btn-flat" id="downloadSearch_resouce_data" data-toggle="tooltip" title="Add" href="<?php echo base_url('resource_schedule/search_res_report');?>"><i class="fa    fa-search" ></i> Search</a>  -->
                    
                    <?php else: ?>
                    
                     <a class="btn btn-info btn-flat" data-toggle="tooltip" title="Refresh" href="<?php echo base_url();?>resource_schedule"><i class="fa fa-lg fa-refresh"></i></a>
                    
                    <?php endif;?> 
		      </div>
              
				<?php if(!empty($_REQUEST['showsearch'])): ?>
                 <div>&nbsp;</div>
                <!-- Search for employee with date wise and client , project wise as well. -->
				<div class="col-md-12">
					<div class="bs-component">
								...
								<div class="tab-pane">
									<div class="row">
										<div class="col-md-2">
											<div class="form-group">
												<label class="control-label">Department</label>
												<select class="form-control" id="department_search" name="department">
...
<script language="javascript" type="text/javascript">
	...
	$("#downloadSearch_resouce_data").click(function() {
		$("#table22excel_all").table2excel({
			...
			filename: "resource_report_<?php echo date("Y-m-d"); ?>", // do not include extension
	...
	$("#send_today_resource_email").click(function (e) {
		e.preventDefault();
		var $btn = $(this);
		var originalHtml = $btn.html();
		$btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> Sending...');
		$.ajax({
			url: "<?php echo base_url('resource_schedule/send_today_resource_schedule_email'); ?>",
			type: "POST",
			// Let jQuery treat response as text to avoid hard JSON parse errors
			dataType: "text",
			success: function (res) {
				var msg = 'Today\'s resource schedule email request completed.';
				try {
					...
					// If response is not valid JSON, keep default friendly message
				}
				alert(msg);
			},
			error: function () {
				alert('Email request completed, but there was a response error. Please check your inbox or contact the administrator.');
			},
			complete: function () {
...
<!-- akhila edited second-sreen-view Inlude Footer here -->
