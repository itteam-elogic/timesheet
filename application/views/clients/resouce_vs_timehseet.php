    ...
    <div class="content-wrapper">
        ...
        <?php
                                                ...
                                                if (!function_exists('rs_vs_ts_compare_manager_names')) {
                                                    $this->load->helper('rs_vs_ts');
                                                }
                                                uasort($managerTotals, function($a, $b) {
                                                    return rs_vs_ts_compare_manager_names($a['manager_name'], $b['manager_name']);
                                                });
                                                $displayGroups = array();
                                        ...
                                        <table class="table table-bordered rs-vs-ts-dept-table" id="rs_vs_ts_dept_totals_table">
                                            <thead>
                                                <tr class="rs-vs-ts-dept-thead">
                                                    ...
                                                    <th class="rs-vs-ts-th-defaulters">Not Filled Timesheet By</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <?php
                                                $managerDefaultersData = isset($managerDefaulters) && is_array($managerDefaulters) ? $managerDefaulters : array();
                                                foreach ($displayGroups as $rowLabel => $totals):
                                                    $resH = $totals['resource_hours'];
                                                    $tsH  = $totals['timesheet_hours'];
                                                    $diff = $tsH - $resH;
                                                    $mgrKey = strtolower(preg_replace('/\s+/', ' ', trim($rowLabel)));
                                                    $defList = isset($managerDefaultersData[$mgrKey]) ? $managerDefaultersData[$mgrKey] : array();
                                                ?>
                                                <tr class="rs-vs-ts-dept-data-row" data-manager-key="<?php echo htmlspecialchars($mgrKey, ENT_QUOTES, 'UTF-8'); ?>">
                                                    ...
                                                    <td class="rs-vs-ts-cell-defaulters">
                                                        <?php if (!empty($defList)): ?>
                                                            <div class="rs-vs-ts-defaulters-wrap">
                                                            <?php foreach ($defList as $defRow):
                                                                ...
                                                                $defDiff  = $defSched - $defTs;
                                                                $tip = 'Resource: ' . number_format($defSched, 2) . ' hrs | Timesheet: ' . number_format($defTs, 2) . ' hrs | Pending: ' . number_format($defDiff, 2) . ' hrs';
                                                            ?>
                                                                <span class="rs-vs-ts-defaulter-chip" title="<?php echo htmlspecialchars($tip, ENT_QUOTES, 'UTF-8'); ?>">
                                                                    <?php echo htmlspecialchars($defName, ENT_QUOTES, 'UTF-8'); ?>
                                                                </span>
                                                            <?php endforeach; ?>
                                                            </div>
                                                        <?php else: ?>
                                                            <span class="rs-vs-ts-defaulters-none">All filled</span>
                                                        <?php endif; ?>
                                                    </td>
                                                </tr>
                                                <?php endforeach; ?>
                                                <tr class="rs-vs-ts-grand-total-row">
                                                    ...
                                                    <td class="rs-vs-ts-cell-defaulters rs-vs-ts-cell-defaulters-total">
                                                        <?php
                                                        $totalDefaulters = 0;
                                                        foreach ($managerDefaultersData as $mgrKeyT => $listT) {
                                                            if (isset($displayGroups) && is_array($displayGroups)) {
                                                                foreach ($displayGroups as $lbl => $_t) {
                                                                    if (strtolower(preg_replace('/\s+/', ' ', trim($lbl))) === $mgrKeyT) {
                                                                        $totalDefaulters += count($listT);
                                                                        break;
                                                                    }
                                                                }
                                                            }
                                                        }
                                                        echo $totalDefaulters > 0 ? ('Total: ' . $totalDefaulters . ' member' . ($totalDefaulters === 1 ? '' : 's')) : 'All members filled';
                                                        ?>
                                                    </td>
                                                </tr>
                                            </tbody>
    ...
    <style>
        ...
        .rs-vs-ts-dept-thead .rs-vs-ts-th-defaulters {
            background: linear-gradient(135deg, #b45309 0%, #d97706 100%) !important;
            color: #fff !important;
            box-shadow: inset 2px 0 0 rgba(255, 255, 255, 0.18);
            position: relative;
        }
        .rs-vs-ts-dept-thead .rs-vs-ts-th-defaulters::before {
            content: "\f0f3";
        ...
        /* Not Filled Timesheet By column */
        .rs-vs-ts-th-defaulters {
            text-align: left;
            width: 340px;
            min-width: 300px;
        }
        .rs-vs-ts-cell-defaulters {
            ...
            border-left: 1px solid #f3e1c2;
        }
        .rs-vs-ts-defaulters-wrap {
            display: flex;
            flex-wrap: wrap;
            gap: 6px 8px;
            line-height: 1.5;
        ...
        .rs-vs-ts-defaulter-chip {
            display: inline-flex;
            align-items: center;
            gap: 6px;
            background: linear-gradient(135deg, #fff4c4 0%, #fde68a 100%);
            color: #6b2d12;
            ...
            line-height: 1.3;
            white-space: nowrap;
            box-shadow: 0 1px 2px rgba(180, 100, 20, 0.10);
            transition: transform 0.15s ease, box-shadow 0.15s ease, background 0.15s ease;
            cursor: default;
        }
        .rs-vs-ts-defaulter-chip:hover {
            background: linear-gradient(135deg, #fde68a 0%, #fbbf24 100%);
            border-color: #d97706;
            box-shadow: 0 2px 6px rgba(180, 100, 20, 0.22);
            transform: translateY(-1px);
        }
        .rs-vs-ts-defaulter-chip::before {
            content: "\f007";
            font-family: "FontAwesome";
            font-weight: normal;
            font-size: 11px;
        ...
        .rs-vs-ts-defaulters-none::before {
            content: "\f00c";
            font-family: "FontAwesome";
            color: #16a34a;
            margin-right: 6px;
        ...
        .rs-vs-ts-cell-defaulters-total {
            background: linear-gradient(180deg, #fff1d6 0%, #ffe2b3 100%) !important;
            font-style: italic;
            color: #7c2d12;
            font-weight: 700 !important;
            letter-spacing: 0.2px;
        }
        .rs-vs-ts-cell-defaulters-total::before {
            content: "\f071";
            font-family: "FontAwesome";
            color: #d97706;
            margin-right: 8px;
        ...
        /* Highlight rows that have defaulters with a subtle stripe */
        .rs-vs-ts-dept-data-row:hover .rs-vs-ts-cell-defaulters {
        ...
        .rs-vs-ts-grand-total-row .rs-vs-ts-cell-defaulters {
    ...
    <script language="javascript" type="text/javascript">
        ...
        <?php
            // Build a JS-ready map of manager_key -> array of { name, scheduled, timesheet }
            $rsVsTsDefaultersJs = array();
            if (isset($managerDefaulters) && is_array($managerDefaulters)) {
                foreach ($managerDefaulters as $mgrKeyJs => $listJs) {
                    $items = array();
                    foreach ($listJs as $defRowJs) {
                        $items[] = array(
                            'name' => isset($defRowJs->member_name) ? trim((string)$defRowJs->member_name) : '',
                            'scheduled' => isset($defRowJs->scheduled_hours) ? (float)$defRowJs->scheduled_hours : 0,
                            'timesheet' => isset($defRowJs->timesheet_hours) ? (float)$defRowJs->timesheet_hours : 0
                        );
                    }
                    $rsVsTsDefaultersJs[$mgrKeyJs] = $items;
                }
            }
        ?>
        var rsVsTsDefaultersByManager = <?php echo json_encode($rsVsTsDefaultersJs); ?>;
        function rsVsTsManagerSortKey(name) {
            var key = (name || '').toString().toLowerCase().replace(/\s+/g, ' ').trim();
            if (key === 'pradip chauhan') {
                return 'syed afsar ';
            }
            return key;
        }
        function rsVsTsCompareManagerNames(a, b) {
            var ka = rsVsTsManagerSortKey(a);
            var kb = rsVsTsManagerSortKey(b);
            if (ka < kb) return -1;
            if (ka > kb) return 1;
            return 0;
        }
        $(document).ready(function() {
            ...
            function rebuildDepartmentSummary() {
                ...
                var displayGroups = {};
                Object.keys(managerTotals).sort(rsVsTsCompareManagerNames).forEach(function(managerName) {
                ...
                function buildDefaultersCell(managerLabel) {
                    var key = (managerLabel || '').toString().toLowerCase().replace(/\s+/g, ' ').trim();
                    var list = rsVsTsDefaultersByManager[key] || [];
                    var $td = $('<td class="rs-vs-ts-cell-defaulters"></td>');
                    if (!list.length) {
                        $td.html('<span class="rs-vs-ts-defaulters-none">All filled</span>');
                        return $td;
                    }
                    var $wrap = $('<div class="rs-vs-ts-defaulters-wrap"></div>');
                    list.forEach(function(item) {
                        var nm = item && item.name ? String(item.name) : '';
                        if (!nm) return;
                        var sched = parseFloat(item.scheduled || 0);
                        var tsH = parseFloat(item.timesheet || 0);
                        var pend = sched - tsH;
                        var tip = 'Resource: ' + sched.toFixed(2) + ' hrs | Timesheet: ' + tsH.toFixed(2) + ' hrs | Pending: ' + pend.toFixed(2) + ' hrs';
                        $wrap.append($('<span class="rs-vs-ts-defaulter-chip"></span>').attr('title', tip).text(nm));
                    });
                    $td.append($wrap);
                    return $td;
                }
                ...
                $.each(displayGroups, function(rowLabel, totals) {
                    ...
                    $tr.append(buildDefaultersCell(rowLabel));
                    $tbody.append($tr);
                });
                ...
                var totalDefaultersCount = 0;
                Object.keys(displayGroups).forEach(function(mgrLbl) {
                    var k = mgrLbl.toLowerCase().replace(/\s+/g, ' ').trim();
                    var list = rsVsTsDefaultersByManager[k] || [];
                    totalDefaultersCount += list.length;
                });
                var totalDefaultersText = totalDefaultersCount > 0
                    ? ('Total: ' + totalDefaultersCount + ' member' + (totalDefaultersCount === 1 ? '' : 's'))
                    : 'All members filled';
                ...
                $grandTr.append($('<td class="rs-vs-ts-cell-defaulters rs-vs-ts-cell-defaulters-total"></td>').text(totalDefaultersText));
        ...
        $("#downloadEmployeeData_total_client_Data").click(function() {
            ...
            $deptTbl.find('tbody tr').each(function() {
                ...
                $(this).find('td').each(function() {
                    var $srcTd = $(this);
                    var cellText = '';
                    var $chips = $srcTd.find('.rs-vs-ts-defaulter-chip');
                    if ($chips.length) {
                        var names = [];
                        $chips.each(function() { names.push($(this).text().trim()); });
                        cellText = names.join(', ');
                    } else {
                        cellText = $srcTd.text().replace(/\s+/g, ' ').trim();
                    }
                    $r.append($('<td></td>').text(cellText).css({ 'padding': '12px 14px', 'border': '1px solid #e8e8e8', 'font-weight': '600' }));
                });
                $body.append($r);
            });
            ...
            // Override 5th column (Not Filled Timesheet By) in export to be wider and left-aligned
            $clone.find('thead th:eq(4)').css({ 'width': '320px', 'text-align': 'left' });
            $clone.find('tbody tr.rs-vs-ts-export-summary-row td:nth-child(5)').css({ 'text-align': 'left', 'white-space': 'normal', 'word-break': 'break-word', 'background-color': '#fff7ed' });
